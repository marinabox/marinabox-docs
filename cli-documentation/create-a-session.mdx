...
```bash
mb local create --env-type=<browser|desktop> [--resolution=<WIDTHxHEIGHTxDEPTH>] [--tag=<TAG>]
```
...
- `--resolution`: (Optional) Sets the resolution of the session in the format `WIDTHxHEIGHTxDEPTH`. Default is `1200x800x24`.
...
```bash
mb local create --env-type=desktop --resolution=1920x1080x24
```
This creates a desktop session with a resolution of 1920x1080 and a depth of 24.
